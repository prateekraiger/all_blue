...
export function BrandValues() {
  return (
    <section className="w-full bg-white border-y border-grey-100">
      <div className="max-w-[1920px] mx-auto px-4 sm:px-6 lg:px-12 py-16 md:py-24">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12 md:gap-16">
          {values.map((value, index) => (
            ...
            >
              <div className="p-3 bg-grey-100 rounded-full group-hover:bg-nike-black group-hover:text-white transition-colors duration-300">
                <value.icon className="w-6 h-6" />
              </div>
              <h3 className="nike-heading text-[18px] md:text-[20px] text-nike-black">
                {value.title}
              </h3>
              <p className="text-[14px] text-grey-500 leading-relaxed font-normal" style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }}>
                {value.description}
              </p>
            </motion.div>
          ))}
